...
folder are mapped to the application's root namespace, 'Mygitcheck'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "Mygitcheck"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'Mygitcheck' in the same way
as `"Mygitcheck/sass/src"`.
