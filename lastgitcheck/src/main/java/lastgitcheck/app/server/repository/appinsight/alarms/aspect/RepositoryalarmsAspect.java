package lastgitcheck.app.server.repository.appinsight.alarms.aspect;
import lastgitcheck.app.server.repository.aspect.RepositoryAspect;
...
public class RepositoryalarmsAspect extends RepositoryAspect {
    ...
    public Object aroundSave(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            Log.out.println("null", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName() + ":" + joinPoint.getSignature().getName());
            object = joinPoint.proceed();
            methodCallDetails.setPostCallDetails(HealthConstants.METHOD_NORMAL_EXECUTION);
            Log.out.println("AISAL347900100", runtimeLogInfoHelper.getRequestHeaderBean(), joinPoint.getSignature().getDeclaringTypeName(), joinPoint.getSignature().getName(), joinPoint.getSignature().getDeclaringTypeName(), methodCallDetails.getExecution_Time());
        } catch (Exception e) {
    ...
    @Pointcut("execution(* lastgitcheck.app.server.repository.appinsight.alarms..save*(..))")
    protected void saveOperation() {
    }

    @Pointcut("execution(* lastgitcheck.app.server.repository.appinsight.alarms..update*(..))")
    protected void updateOperation() {
    }

    @Pointcut("execution(* lastgitcheck.app.server.repository.appinsight.alarms..delete*(..))")
    protected void deleteOperation() {
    }

    @Pointcut("execution(* lastgitcheck.app.server.repository.appinsight.alarms..find*(..)) && ! alarmRepofindOperation()")
    protected void findOperation() {
    }

    @Pointcut("execution(* lastgitcheck.app.server.repository.appinsight.alarms..get*(..)) && ! alarmRepofindOperation()")
    protected void getOperation() {
    }

    @Pointcut("execution(* lastgitcheck.app.server.repository.appinsight.alarms..*(..)) && ! findOperation() && ! saveOperation() && ! updateOperation() && ! deleteOperation() && ! getOperation() && ! alarmRepofindOperation()")
    protected void allOperation() {
    }

    @Pointcut("execution(* lastgitcheck.app.server.repository.appinsight.alarms.ArtLogAlarmRepository..getVersionNumber(..)) || execution(* lastgitcheck.app.server.repository.appinsight.alarms.ArtLogAlarmRepository..getAlarmDataByType(..))")
