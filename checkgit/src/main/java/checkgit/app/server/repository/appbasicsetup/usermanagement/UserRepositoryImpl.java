...
public class UserRepositoryImpl extends SearchInterfaceImpl implements UserRepository<User> {
    ...
    public List<User> findAll() throws Exception {
        javax.persistence.EntityManager emanager = emfResource.getResource();
        java.util.List<checkgit.app.shared.appbasicsetup.usermanagement.User> query = emanager.createQuery("select u from User u where u.systemInfo.activeStatus=1").getResultList();
    ...
    public List<User> save(List<User> entity) throws Exception {
        javax.persistence.EntityManager emanager = emfResource.getResource();
        for (int i = 0; i < entity.size(); i++) {
            checkgit.app.shared.appbasicsetup.usermanagement.User obj = entity.get(i);
    ...
    public void delete(String id) throws Exception {
        javax.persistence.EntityManager emanager = emfResource.getResource();
        checkgit.app.shared.appbasicsetup.usermanagement.User s = emanager.find(checkgit.app.shared.appbasicsetup.usermanagement.User.class, id);
        emanager.remove(s);
    ...
    public void deletePassRecovery(List<PassRecovery> passrecovery) {
        javax.persistence.EntityManager emanager = emfResource.getResource();
        for (checkgit.app.shared.appbasicsetup.usermanagement.PassRecovery _passrecovery : passrecovery) {
            checkgit.app.shared.appbasicsetup.usermanagement.PassRecovery s = emanager.find(checkgit.app.shared.appbasicsetup.usermanagement.PassRecovery.class, _passrecovery.getPassRecoveryId());
            emanager.remove(s);
        }
    }
    ...
    public List<User> findByUserAccessLevelId(String userAccessLevelId) throws Exception {
        ...
        java.util.List<checkgit.app.shared.appbasicsetup.usermanagement.User> listOfUser = query.getResultList();
    ...
    public User findById(String userId) throws Exception {
        ...
        javax.persistence.Query query = emanager.createNamedQuery("User.findById");
        query.setParameter("userId", userId);
        checkgit.app.shared.appbasicsetup.usermanagement.User listOfUser = (checkgit.app.shared.appbasicsetup.usermanagement.User) query.getSingleResult();
