Ext.define('Checkgit.checkgit.web.checkgit.controller.organization.locationmanagement.CheckingMainController', {
     extend: 'Checkgit.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.CheckingMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#CheckingTree');
     ...
     refreshMainForm: function(but, data, method) {
          var formPanel = this.view.down('#CheckingForm');
          this.updateFormUI(formPanel, 'Save', true); /** Adding data to tree and grid */
          if (data != null) {
               var grid = this.view.down('#CheckingGrid');
               var tree = this.view.down('#CheckingTree');
               if (method == 'PUT') {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#CheckingForm');
          var grid = this.view.down('#CheckingGrid');
          var tree = this.view.down('#CheckingTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               ...
               var gridPanel = this.view.up().down('#CheckingGrid');
               ...
               var formPanel = this.getView().up().down('#CheckingForm');
               jsonData = {
                    'findKey': primaryKey
               }
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) {
          ...
          var treePanel = this.view.up().up().down('#CheckingTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#CheckingForm').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#CheckingGrid').store;
          gridStore.removeAll();
